...
{
    public class UIKatanaryEntryInfoPage : UIPanel
    {
        ...
        public UIKatanaryEntryInfoPage()
        {
            Width.Set(230f, 0f);
            Height.Set(0f, 1f);
            SetPadding(0f);
        ...
        public void FillInfoForEntry(KatanaryEntryItem entry, ExtraBestiaryInfoPageInformation extraInfo)
        {
            _list.Clear();
            if (entry != null)
            {
        ...
        private BestiaryUICollectionInfo GetUICollectionInfo(KatanaryEntryItem entry, ExtraBestiaryInfoPageInformation extraInfo)
        {
            BestiaryUICollectionInfo result = entry.UIInfoProvider?.GetEntryUICollectionInfo() ?? default;
            result.OwnerEntry = entry;
            return result;
        }

        private void AddInfoToList(KatanaryEntryItem entry, ExtraBestiaryInfoPageInformation extraInfo)
        {
        ...
        private BestiaryInfoCategory GetBestiaryInfoCategory(IBestiaryInfoElement element)
        {
            ...
            {
                return (BestiaryInfoCategory)categorizedElement.ElementCategory;
            }
            ...
            if (element is KatanaStatsReportInfoElement || element is NPCKillCounterInfoElement)
