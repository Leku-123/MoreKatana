...
namespace MoreKatana.UI.Katanary
{
    ...
    {
        public KatanaryState(KatanaryDataBase database)
        {
            _filterer.SetSearchFilterObject(new Filters.BySearch());
            _originalEntriesList = new((IEnumerable<BestiaryEntry>)database.Entries);
            _workingSetEntries = new(_originalEntriesList);
        ...
        private void BuildPage()
        {
            ...
            UIKatanaryEntryGrid uIBestiaryEntryGrid = new UIKatanaryEntryGrid(_workingSetEntries, new UIElement.MouseEvent(Click_SelectEntryButton));
            uIElement5.Append(uIBestiaryEntryGrid);
            _entryGrid = uIBestiaryEntryGrid;
            _entryGrid.OnGridContentsChanged += UpdateKatanaryGridRange;
        ...
        private void FillProgressBottomBar(UIElement container)
        {
            ...
            KatanaryBar uIColoredSliderSimple = new(ModContent.GetInstance<KatanaryDataBase>())
            {
                Width = new StyleDimension(0f, 1f),
        ...
        private string GetCompletionPercentText()
        {
            ...
            return Language.GetTextValueWith("KatanaryInfo.PercentCollected", new
            {
                Percent = percent
            });
        }
        ...
        private void OpenVirtualKeyboardWhenNeeded()
        {
            ...
            UIVirtualKeyboard uIVirtualKeyboard = new UIVirtualKeyboard(Language.GetText("UI.PlayerNameSlot").Value, _searchString, new UIVirtualKeyboard.KeyboardSubmitEvent(OnFinishedSettingName), new Action(GoBackHere), 0, true);
        ...
        private void OnFinishedSettingName(string name)
        {
            ...
            _searchBar.SetContents(contents, false);
            GoBackHere();
        }
        ...
        private void FilterEntries()
        {
            ...
            _workingSetEntries.AddRange(_originalEntriesList.Where(new Func<KatanaryEntryItem, bool>(_filterer.FitsFilter)));
        }

        private void SortEntries()
        {
            foreach (KatanaryEntryItem bestiaryEntry in _workingSetEntries)
            {
        ...
        public void UpdateKatanaryGridRange()
        {
        ...
        private void UpdateKatanaryContents()
        {
        ...
        {
            UIKatanaryEntryButton uIBestiaryEntryButton = (UIKatanaryEntryButton)listeningElement;
        ...
        private void SelectEntryButton(UIKatanaryEntryButton button)
        {
            DeselectEntryButton();
        ...
        {
            _infoSpace.FillInfoForEntry(null, default);
        }
        ...
        public BestiaryUnlockProgressReport GetUnlockProgress()
        {
            float num = 0f;
            int num2 = 0;
            List<KatanaryEntryItem> originalEntriesList = _originalEntriesList;
        ...
        private void SetupPointsForEntryGrid(ref int currentID, List<SnapPoint> pts, out int gridWidth, out int gridHeight, out UILinkPoint[,] gridPoints)
        {
            ...
            int num3;
            _entryGrid.GetEntriesToShow(out gridWidth, out gridHeight, out num3);
            ...
            for (int i = 0; i < orderedPointsByCategoryName.Count; i++)
            {
                ...
                num3 = currentID;
                currentID = num3 + 1;
                array[num4, num5] = MakeLinkPointFromSnapPoint(num3, orderedPointsByCategoryName[i]);
            }
            for (int j = 0; j < gridWidth; j++)
            {
        ...
        private void SetupPointsForFilterGrid(ref int currentID, List<SnapPoint> pts, out int gridWidth, out int gridHeight, out UILinkPoint[,] gridPoints)
        {
            ...
            int num3;
            _filteringGrid.GetEntriesToShow(out gridWidth, out gridHeight, out num3);
            gridPoints = new UILinkPoint[gridWidth, gridHeight];
            for (int i = 0; i < orderedPointsByCategoryName.Count; i++)
            {
                int num = i % gridWidth;
        ...
        private void SetupPointsForSortingGrid(ref int currentID, List<SnapPoint> pts, out int gridWidth, out int gridHeight, out UILinkPoint[,] gridPoints)
        {
            ...
            int num3;
            _sortingGrid.GetEntriesToShow(out gridWidth, out gridHeight, out num3);
            gridPoints = new UILinkPoint[gridWidth, gridHeight];
            for (int i = 0; i < orderedPointsByCategoryName.Count; i++)
            {
                int num = i % gridWidth;
        ...
        private UIKatanaryEntryButton _selectedEntryButton;


        private List<KatanaryEntryItem> _originalEntriesList;
        ...
        private EntryFilterer<KatanaryEntryItem, IBestiaryEntryFilter> _filterer = new EntryFilterer<KatanaryEntryItem, IBestiaryEntryFilter>();


        private EntrySorter<KatanaryEntryItem, IBestiarySortStep> _sorter = new EntrySorter<KatanaryEntryItem, IBestiarySortStep>();


        private UIKatanaryEntryGrid _entryGrid;
        ...
        private KatanaryBar _unlocksProgressBar;
